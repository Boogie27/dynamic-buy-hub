...
$(window).ready(function(){
        ...
        // AJAX CALL THAT HANDLES THE SEACRCH FIELD
       
        $("#searchForm #search").keyup(function(event){
            $("#search-items").show();
         ...
         $("#search-items").on("click", "li", function(event){
            $("#searchForm #search").val($(this).text())
            $("#search-items").html("");
            $("#search-items").hide();
         });

         $(document).click(function(event){
             items = event.target.id;
             if(items != "search"){
                $("#search-items").html("");
                $("#search-items").hide();
             }
...
// ====================================================================
//          AJAX CALL THAT HANDLES THE ADD TO CART 
// ====================================================================



         function cartQty(){
              var cartQty = $(".mysection #cartQty");
              var myCartHeader = $(".myCart-header span");
                    quantity = "cartQty";
                    $.ajax({
                         url: "ajax.php",
                         method: "post",
                         data: {cartQty:quantity},
                         success: function(response){
                             $(cartQty).html(response);
                             $(".top-link .cartq").html(response);
                             $(".myCart-header .cartQty").html(response);
                             $(myCartHeader).html("My Cart ("+response+")");
                         }
                    });
         }     
         cartQty();
         ...
         function cartItems(){
              var cartItems = $(".right-bar .cartItems");
              var myCart = $(".myCart .cartImages");
         ...
         cartItems();
          ...
          var setTime;
          var addToCart = $(".addToCartFrom");
         
        
          if(addToCart.length > 0){
            $.each(addToCart, function(index, current){
                    var button = $(current).find(".addToCart");
                    var itemID = $(current).find(".itemID").val();
                    var qty = null;

                    button.click(function(e){
                        var conatiner =  $(".alert-container .cartAlert");
                        conatiner.html("");
                        e.preventDefault();
                        $.ajax({
                            url: "ajax.php",
                            ...
                            success: function(response){
                                ...
                                $(".alert-container").show();
                                 ...
                                 if(response == "success"){
                                      var alertItem =  `<span class="alert-logo"><i class="fas fa-check"></i></span>
                                                       <span class="alertContent">Item has been added to cart successfully!</span>`;
                                                       conatiner.append(alertItem);
                                 }else if(response == "outOfStock"){
                                    var alertItem =  `<span class="alert-logo-red"><i class="fas fa-times"></i></span>
                                                      <span class="alertContent text-danger">Item is out of stuck!</span>`;
                                                    conatiner.append(alertItem);
                                 }
                                if($(".alert-container").show()){
                                    setTime =  setTimeout(hideAlert, 3000);
                                }
                            }
...
// ========================================================================
// FUNCTION THAT REMOVES ALERT CART MESSAGE
// ========================================================================
          var cartAlert = $(".error-alert");
...
// ========================================================================
          ...
          var formSignupButton = $(".signUpAction");
          signupForm.submit(function(e){
            e.preventDefault();
                        var name = $(".signupForm #name").val();
                        var email = $(".signupForm #email").val();
                        var password = $(".signupForm #password").val();
                        var confimPassword = $(".signupForm #confirm_password").val();
                        var signupButton = $(".signupForm #signupButton").val();
...
var loginAction = $(".loginAction");
    loginAction.click(function(){
        $(loginForm).submit(function(e){
                 e.preventDefault();
                 var email = $(".loginForm #logineMmail").val();
                 var password = $(".loginForm #loginPassowrd").val();
                 $.ajax({
                     url: "ajax.php",
                     method: "post",
                     data: {
...
// ============================================================================
//      FUNCTION THAT CHANGES USER PASSWORD                           
// ============================================================================ 
...
// ============================================================================
  ...
  var logUserOut = $(".logUserOut");
       $(logUserOut).click(function(e){
           e.preventDefault();
           $.ajax({
                url: "ajax.php",
                method: "post",
                data: {logUserOut:"logUserOut"},
                success: function(response){
...
var deleteButton = $(".deleteAccountForm #deleteButton");
...
shippingContainer.click(function(vent){
    $(this).html($(this).html() === "open Shipping" ? "close Shipping" : "open Shipping");
...
//   ==============================================================
//             FUNCTION THAT SLIDE ELEMENT TO THE TOP ONSCROLL
// ================================================================
...
var editCancle = $(".edit-section .fa-times");

var deleteAccountForm = $(".deleteAccount-section");
var deleteAccountCancle = $(".deleteAccount-section .fa-times");
        ...
        function openForm(value){
            var type;
                ...
                if(value == "signup-form"){
                      ...
                      type = changePassword;
                }else if(value === "edit-form"){
                    type = editForm;
                }else if(value === "deleteAccount-form"){
            ...
            $(current).click(function(event){
                var dataType = $(event.target).attr("data-type");
                    if(dataType){
                        ...
                        if(dataType === "edit-form"){
                            openForm(dataType)
                        }
                        if(dataType === "deleteAccount-form"){
                            openForm(dataType)
                        }
                    }
            });
        ...
        var closeType;
                if(value == "closeSignup"){
                    closeType = signupForm;
                     ...
                     closeType = editForm;
                }else if(value === "closeDeleteAccount"){
                    closeType = deleteAccountForm;
               }
            ...
            if(editCancle){
                $(editCancle).click(function(event){
                    var type = $(event.target).attr("id")
                closeSignUp(type);
                });
            }
            if(deleteAccountCancle){
                $(deleteAccountCancle).click(function(event){
                    var type = $(event.target).attr("id")
                closeSignUp(type);
                });
            }
